...
- 已上传的 full-v3 artifact 是一份独立的 64-bin categorical snapshot；本地 R2 recovery 则证明 V1 runtime 曾实际恢复并服务过另一份已发布 release。
...
当前可见的本地 V1 混合训练 release 是两次联合更新、每次 9 条 verified replay action 加 1 条 Mathlib action。这是有效的机制级训练，不是对 full-v3 本身或大规模能力的结论。
...
| [08 上传的 full-v3 artifact](08-uploaded-full-v3-artifact.md) | 64-bin snapshot 的可验证身份与评估边界 |
